...
def denoising_loop(model, params, diff_params, images_in_0, dtype = jnp.float32):
    """
    Full on device ddim sample loop, non-respaced version
    """
    def one_step(i, images_in_step):
        embed_in = jnp.array([999 - i]) # TODO more than one
    ...
    return jax.lax.fori_loop(0, 1000, one_step, images_in_0) # TODO len
